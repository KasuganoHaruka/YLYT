...
namespace _05.Toolkit.JwtToken
{
    ...
    public class JwtToken
    {
        ...
        /// <returns></returns>
        public static string IssueJWT(string accountJson, TimeSpan expiresSliding, TimeSpan expiresAbsoulte)
        {
            ...
            Claim[] claims = new Claim[]
          {
            ...
            JwtSecurityToken token = new JwtSecurityToken(
                issuer: AppSettingManager.Configuration["AppConfig:JwtKeyName"],//jwt签发者,非必须
                audience: AppSettingManager.Configuration["AppConfig:JwtKeyName"],//token的接收该方，非必须
                claims: claims,//声明集合
                expires: UTC.AddHours(12),//指定token的生命周期，unix时间戳格式,非必须
                signingCredentials: new SigningCredentials(new SymmetricSecurityKey(Encoding.ASCII.GetBytes(AppSettingManager.Configuration["AppConfig:JwtSecurityKey"])),
                                                            SecurityAlgorithms.HmacSha256));//使用私钥进行签名加密

            var encodedJwt = new JwtSecurityTokenHandler().WriteToken(token);//生成最后的JWT字符串

            RayPIMemoryCache.AddMemoryCache(encodedJwt, accountJson, expiresSliding, expiresAbsoulte);//将JWT字符串和tokenModel作为key和value存入缓存
            return encodedJwt;
        }
